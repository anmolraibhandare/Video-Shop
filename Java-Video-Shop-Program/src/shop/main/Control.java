        ...
        public void run() {
          // TODO
        	if(_inventory.size()>0) {
        		Iterator<Record> iteratorToPrint = _inventory.iterator(new java.util.Comparator<Record>() {
        	...
        	int i = 1;
        	stringBuilder.append("Print top ten all time rentals in order \n");
        	while(iteratorToPrint.hasNext() && i <11) {
        		Record record = iteratorToPrint.next();
        		stringBuilder.append(" " + record.video().title() + " [" + record.numOwned() + "]\n");
        		i++;
        	}
        	_ui.displayMessage(stringBuilder.toString());
        } else {
